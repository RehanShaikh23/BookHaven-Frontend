...
const Dashboard = () => {
  ...
  const rawCartValue = getCartTotal;
  
  
  const cartValue = Number(rawCartValue) || 0;
          ...
          <div className="bg-white rounded-xl shadow-md p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-coffee-900">${formatCurrency(totalValue)}</p>
              </div>
            ...
            <div className="mt-4">
              <p className="text-green-600 text-sm font-medium">
                Average: ${totalBooks > 0 ? formatCurrency(totalValue / totalBooks) : '0.00'} per book
              </p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-md p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-coffee-900">{(Number(averageRating) || 0).toFixed(1)}</p>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-md p-6">
            ...
            <div className="mt-4">
              <p className="text-orange-600 text-sm font-medium">
                Total: ${formatCurrency(cartValue)}
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              {quickActions.map((action, index) => (
                <Link
                  key={index}
                  to={action.link}
                  className="bg-white rounded-xl shadow-md p-6 transition-all duration-300 ease-out group transform hover:-translate-y-1 hover:shadow-xl ring-1 ring-transparent hover:ring-2 hover:ring-coffee-200"
                >
                  <div className="flex items-center space-x-4">
                    <div className={`p-3 rounded-full text-white transition-all duration-300 transform group-hover:scale-110 group-hover:rotate-3 ${action.color}`}>
                    ...
                    <div>
                      ...
                      <div className="mt-3 h-1 w-0 group-hover:w-24 transition-all duration-500 bg-gradient-to-r from-coffee-400 to-coffee-600 rounded-full"></div>
                    </div>
                  </div>
                </Link>
              ))}
                  ...
                  {recentBooks.map((book) => (
                    <div key={book.id} className="bg-white rounded-xl shadow-md p-4 flex items-center space-x-4">
                      ...
                      <div className="flex items-center space-x-1">
                        ...
                        <span className="text-sm text-coffee-600">{(Number(book.rating) || 0).toFixed(1)}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="bg-white rounded-xl shadow-md p-6">
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-coffee-900">${formatCurrency(cartValue)}</span>
                </div>
                <div className="flex justify-between">
